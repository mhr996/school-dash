...
const AddGuide = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { data: guideInsertData, error } = await supabase
                .from('guides')
                .insert([
                    {
                        ...guideData,
            ...
            if (profilePictureFile && guideInsertData) {
                const uploadResult = await uploadServiceProfilePicture(profilePictureFile, 'guides', guideInsertData.id);

                if ('error' in uploadResult) {
                    console.error('Error uploading profile picture:', uploadResult.error);
                    // Don't fail the whole operation, just log the error
                } else {
                    // Update guide with profile picture URL
                    await supabase.from('guides').update({ profile_picture_url: uploadResult.path }).eq('id', guideInsertData.id);
                }
            }
