...
export default function EditDestinationPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const { data: propertyLinks, error: propError } = await supabase.from('destination_properties_link').select('property_id').eq('destination_id', params.id);
                if (propError) throw propError;
                setProperties((propertyLinks || []).map((link: any) => link.property_id));

                // Load suitable-for from junction table
                const { data: suitableLinks, error: suitableError } = await supabase.from('destination_suitable_for_link').select('suitable_for_id').eq('destination_id', params.id);
                if (suitableError) throw suitableError;
                setSuitable((suitableLinks || []).map((link: any) => link.suitable_for_id));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Update junction tables for properties
            // Delete existing property links
            const { error: deletePropError } = await supabase.from('destination_properties_link').delete().eq('destination_id', params.id);
            ...
            // Update junction tables for suitable-for
            // Delete existing suitable-for links
            const { error: deleteSuitableError } = await supabase.from('destination_suitable_for_link').delete().eq('destination_id', params.id);
            if (deleteSuitableError) throw deleteSuitableError;

            // Insert new suitable-for links
