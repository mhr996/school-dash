...
interface EducationProgramService {
    id?: string;
...
interface EducationProgramTabsProps {
    programId: string;
    programData: {
        name: string;
        description?: string;
...
export default function EducationProgramTabs({ programId, programData, onUpdate, isServiceProvider = false }: EducationProgramTabsProps) {
    const { t } = getTranslation();
    const [services, setServices] = useState<EducationProgramService[]>([]);
    ...
    const fetchServices = async () => {
        try {
            setLoading(true);
            const { data, error } = await supabase.from('education_program_services').select('*').eq('education_program_id', programId).order('created_at', { ascending: false });
    ...
    const handleServiceChange = (index: number, field: keyof EducationProgramService, value: any) => {
    ...
    const handleServiceIconUpload = async (index: number, file: File) => {
        ...
        try {
            const result = await uploadSubServiceIcon({
                serviceType: 'education_program_services',
                parentServiceId: programId,
    ...
    const handleServiceIconDelete = async (index: number) => {
        ...
        try {
            const result = await deleteSubServiceIcon('education_program_services', programId, service.id);
    ...
    const confirmDeleteService = async () => {
        ...
        if (service.id) {
            ...
            if (service.icon_path) {
                try {
                    await deleteSubServiceIcon('education_program_services', programId, service.id);
                } catch (error) {
            ...
            try {
                const { error } = await supabase.from('education_program_services').delete().eq('id', service.id);
        ...
        <div>
                                        ...
                                        <div className="flex items-start gap-4">
                                            ...
                                            {basicInfo.image && (
                                                <div className="flex-shrink-0">
                                                    <img
                                                        src={basicInfo.image}
                                                        alt={t('current_image') || 'Current image'}
                                            ...
                                            <div className="flex-1">
                                                ...
                                                />
                                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">{t('supported_formats') || 'Supported formats'}: JPG, PNG, GIF</p>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <Tab.Panel>
                        <div className="panel">
                            <div className="mb-6 flex items-center justify-between">
                                <div>
                                    <h3 className="text-xl font-bold">{t('services_provided') || 'Services Provided'}</h3>
                                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('manage_sub_services_pricing') || 'Manage sub-services and pricing'}</p>
                                </div>
                            ...
                            {loading ? (
                                ...
                                <div className="text-center py-16">
                                    ...
                                    <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-2">{t('no_services_yet') || 'No services yet'}</h3>
                                    <p className="text-gray-500 dark:text-gray-400 mb-6">{t('click_add_service_to_start') || 'Click "Add Service" to start'}</p>
                                ...
                                <div className="space-y-3">
                                        ...
                                        <table className="table-auto w-full">
                                            <thead>
                                                <tr className="bg-gray-50 dark:bg-gray-800">
                                                    ...
                                                    <th className="px-4 py-3  text-sm font-semibold text-gray-700 dark:text-gray-300">{t('icon') || 'Icon'}</th>
                                                    ...
                                                    <th className="px-4 py-3 text-center text-sm font-semibold text-gray-700 dark:text-gray-300">{t('actions') || 'Actions'}</th>
                                                </tr>
                                            </thead>
                                                ...
                                                {services.map((service, index) => (
                                                    <tr key={service.id || `new-${index}`} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                {service.icon_path || service.pendingIconPreview ? (
                                                                    <div className="relative group">
                                                                        <img
                                                                            src={service.pendingIconPreview || getSubServiceIconUrl('education_program_services', service.icon_path || null) || ''}
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            title={t('delete_icon') || 'Delete icon'}
                                                                        >
                                                                            ×
                                                                        </button>
                                                                    </div>
                                                        ...
                                                        <td className="px-4 py-3 text-center">
                                                            <button onClick={() => handleDeleteService(index)} className="btn btn-sm btn-outline-danger" title={t('delete') || 'Delete'}>
                                    ...
                                    <div className="flex items-center justify-between pt-6 border-t border-gray-200 dark:border-gray-700">
                                        <p className="text-sm text-gray-600 dark:text-gray-400">
                                            {services.length} {services.length === 1 ? t('service') || 'Service' : t('services') || 'Services'}
                                        </p>
                                        <button onClick={handleSaveServices} disabled={saving} className="btn btn-primary gap-2">
                                            <IconSave className="w-5 h-5" />
                                            {saving ? t('saving') || 'Saving...' : t('save_all_services') || 'Save Services'}
                                        </button>
                                    </div>
                                </div>
                            )}
            ...
            >
                <div className="fixed top-20 right-5 z-[9999] max-w-md">
                    <div className={`rounded-lg p-4 shadow-lg ${alertType === 'success' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'} flex items-start justify-between`}>
                        <p className="flex-1 text-sm">{alertMessage}</p>
                        <button onClick={() => setShowAlert(false)} className="ml-3 inline-flex text-white hover:text-gray-200 focus:outline-none">
